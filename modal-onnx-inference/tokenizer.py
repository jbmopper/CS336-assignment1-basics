"""Minimal BPE tokenizer runtime for Modal inference."""
...
def load_bpe(tokenizer_dir: str | Path) -> tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
    ...
    with (root / "vocab.json").open("r", encoding="latin-1") as f:
        vocab_str = json.load(f)
    vocab = {int(k): v.encode("latin-1") for k, v in vocab_str.items()}
    with (root / "merges.pkl").open("rb") as f:
...
class Tokenizer:
    ...
    def encode(self, text: str) -> list[int]:
        encoded: list[int] = []
        parts = re.split(self._special_pattern, text) if self._special_pattern else [text]
